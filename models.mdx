...
| `Qwen/Qwen3.6-35B-A3B-FP8` | Qwen | 16K | ~200 @ 480p | Capped by context |
...
| `Hcompany/Holo3-35B-A3B` | H Company | 16K | ~200 @ 480p | Capped by context |
...
<AccordionGroup>
  <Accordion title="Context" icon="ruler">
    The context window length we run the model with. Visual tokens, text input, and text output all share this budget.
  </Accordion>

  <Accordion title="Tokens / frame — Qwen and Holo3" icon="image">
    Qwen3.6 and Holo3 use the same image processor as the Qwen3 line: patch 16, `temporal_patch_size=2`, `spatial_merge_size=2`. The formula:
...
### Interleaved text and video

Every active model can mix text segments between visual tokens inside a single message, instead of forcing all visual content into one block followed by text.
...
<Visibility for="agents">
  ...
  3. From the hosted set, pick a model `id` whose properties match the workload (see the [Overshoot-hosted](#overshoot-hosted) table).
