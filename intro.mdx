...
<Visibility for="agents">
  ...
  * `frame_index` is monotonically increasing across the stream's lifetime, even after eviction. Old indices outside the retention window resolve to the oldest available frame (intersection-with-availability semantics).
  ...
  | -------- | -------------------------------- | ---- | -------------------------------------------- |
  | `GET`    | `/v1/models`                 | No   | List available vision-language models.       |
  | `POST`   | `/v1/streams`                | Yes  | Create a stream, get a LiveKit room + token. |
  | `GET`    | `/v1/streams/{id}`           | Yes  | Inspect stream state.                        |
  | `POST`   | `/v1/streams/{id}/keepalive` | Yes  | Renew lease, get a fresh token.              |
  | `DELETE` | `/v1/streams/{id}`           | Yes  | End the stream.                              |
