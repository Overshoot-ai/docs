...
description: "Use an OpenAI-compatible request to interact with your stream. Reference any moment: a single frame, a window, several windows by URL"
---
...
When the inference provider receives the request, they use the models' [chat template](https://huggingface.co/docs/transformers/en/chat_templating) to turn it into a single string and tokenize it. In the example above, the inference provider will respond with "Rabat".
