...
A `Stream` is a leased session: it holds the state of your live feed as long as it's alive. Read on for how to keep one alive — and how to kill it.

## Stream state

The `Stream` holds your video as long as it's alive. If you push no frames, it sits empty. To check the state — frames received, recent FPS, time to expiry — call:
...
When Overshoot starts ingesting your stream, we run a fair amount of processing in the background to make every frame addressable for any model. To avoid leaking sessions, a stream expires 5 minutes after the last keepalive. Renew it by calling `/keepalive` with your `stream_id` regularly — every 2 minutes is a safe cadence.
