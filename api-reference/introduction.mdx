...
The Overshoot API uses bearer tokens for authentication. Include your API key in the `Authorization` header on every authenticated request:
...
API keys are prefixed with `ovs-` and can be created in the [Overshoot dashboard](https://overshoot.ai/dashboard).

[List models](/api-reference/list-models) is the only unauthenticated endpoint.

<Note>
  Calls using your API key can incur costs. Treat keys as secrets and never commit them to source control or expose them in client-side code.
</Note>
