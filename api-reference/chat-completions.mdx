...
Both wrap an HTTPS URL of the form:

```
https://api.overshoot.ai/v1beta/streams/{stream_id}/media?<query>
```

The `<query>` is what selects the moment you want. Different keys for single frames vs. windows.
...
```json
{ "type": "image_url",
  "image_url": { "url": "https://api.overshoot.ai/v1beta/streams/{id}/media?frame_index=-1" }
}
```
...
```json
{ "type": "video_url",
  "video_url": { "url": "https://api.overshoot.ai/v1beta/streams/{id}/media?start_offset_ms=-5000&max_fps=2" }
}
```
...
```json
{
  ...
  "messages": [{
    ...
    "content": [
      ...
      { "type": "video_url",
        "video_url": { "url": "https://api.overshoot.ai/v1beta/streams/{id}/media?start_offset_ms=-5000" }
      }
    ]
  }]
}
